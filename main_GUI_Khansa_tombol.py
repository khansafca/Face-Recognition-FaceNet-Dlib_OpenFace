...
class CameraGUI:
    ...
    def setup_gui(self):
        ...
        self.register_button = tk.Button(self.master, text="Register", command=self.register, font=("Arial", 15, "bold"), height=1, width=14)
        self.register_button.place(x=135, y=430)


    def register(self):
        self.name_label = tk.Label(self.master, text="Enter NIM:", font=("Arial", 15), height=1, width=8)
        self.name_label.place(x=170, y=470)
        
        self.name_entry = tk.Entry(self.master, font=("Arial", 15, 'bold'), justify="center")
        self.name_entry.place(x=120, y=495)

        self.name_button = tk.Button(self.master, text="Set NIM", command=self.set_name, font=("Arial", 15), height=1, width=8)
        self.name_button.place(x=157, y=530)

        self.camera_button = tk.Button(self.master, text="Open Camera", command=lambda: self.open_camera(CAMERA_URL), font=("Arial", 15), height=1, width=16)
        self.camera_button.place(x=125, y=575)
...
def create_train_buttons(root):
    ...
    for text, command in buttons:
        button = tk.Button(root, text=text, command=command, font=("Arial", 15), width=button_width)
        button.place(x=400, y=y_position)
        y_position += 50
...
def create_face_recognition(root, url):
    ...
    for text, command in buttons:
        button = tk.Button(root, text=text, command=command, font=("Arial", 15), width=button_width)
        button.place(x=697, y=y_position)
        y_position += 50
...
if __name__ == "__main__":
    ...
    tk.Button(root, text="Train The Data", command=lambda: create_train_buttons(root), font=("Arial", 15, "bold"), height=1, width=17).place(x=410, y=430)
    tk.Button(root, text="Start Recognize", command=lambda: create_face_recognition(root, CAMERA_URL), font=("Arial", 15, "bold"), height=1, width=19).place(x=690, y=430)
    tk.Button(root, text="Stop Recognition", command=stop_face_recognition, font=("Arial", 15, "bold"), height=1, width=15).place(x=700, y=80)
