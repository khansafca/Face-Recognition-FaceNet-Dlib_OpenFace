...
save_path = '/Users/khansafca/Documents/gui_fixed/openface'

print("Loading face detection model")
proto_path = os.path.join(curr_path, 'openface', 'model', 'deploy.prototxt')
model_path = os.path.join(curr_path, 'openface', 'model', 'res10_300x300_ssd_iter_140000.caffemodel')
face_detector = cv2.dnn.readNetFromCaffe(prototxt=proto_path, caffeModel=model_path)

print("Loading face recognition model")
recognition_model = os.path.join(curr_path, 'openface', 'model', 'openface_nn4.small2.v1.t7')
...
recognizer_path = os.path.join(save_path, 'recognizer_openface.pickle')
with open(recognizer_path, "wb") as f:
    f.write(pickle.dumps(recognizer))

# Save the label encoder
le_path = os.path.join(save_path, 'le_openface.pickle')
